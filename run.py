...
from evaluation import DEFAULT_PHASE2_REPORT_PATH, evaluate_phase2_and_write_report
...
def run_exploration_phase(image_path: str) -> Tuple[BlindKnowledge, List[Cell]]:
    ...
    return knowledge, discovered_path
...
def run_endgame_phase(
    ...
    report = evaluate_phase2_and_write_report(
    ...
    print(f"Phase 2 report saved to {DEFAULT_PHASE2_REPORT_PATH}.", flush=True)
...
def main() -> None:
    ...
    knowledge, discovered_path = run_exploration_phase(IMAGE_PATH)
    qlearner = load_qlearner(QTABLE_PATH)
    run_endgame_phase(IMAGE_PATH, knowledge, discovered_path, qlearner)
